...
```{r}
ggplot(mobility.data, aes(x = date, y = rolling_avg)) +
...
```{r describe transit scores for each county}

# Issue is, lots of counties so depends how granular we need our descriptive analysis to be. 
# Lets look by state first since laws usually run by state

create_histogram <- function(data, state_name) {
  # Filter the data for the given state
  state_data <- data %>% filter(state == state_name)%>% 
    summarise(transit.scores = mean(transit.scores, na.rm = TRUE))              # i can't remember what I was doing here 
  
  if(nrow(state_data) == 0) {
    message("No data available for state: ", state_name)
    return(NULL)
  }
  
  ggplot(state_data, aes(x = transit_scores)) +
    geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.7) +
    labs(title = paste("Distribution of Average Transit Scores in", state_name),
         x = "Transit Scores",
         y = "Frequency") +
    theme_minimal()
}
...
```{r}
unique_states <- unique(covid.data$state)

# Loop through each state and create a histogram
for (state in unique_states) {
  print(create_histogram(covid.data, state))
}
```
